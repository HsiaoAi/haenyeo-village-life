/* ============================================================
   HAENYEO VILLAGE LIFE — GHIBLI STYLE renderer
   A soft painterly anime repaint. Classic script, shares global
   scope with game.js. Same function signatures as render.js so
...
/* Painterly anime light: warmer, more luminous, softer night.
   Lower alphas keep the world bright; sunrise/sunset push warm
   peach + rose; night is a gentle indigo rather than near-black. */
const DAYKEYS=[
  {t:0,    c:[34,46,92,.40],   glow:0},
  {t:330,  c:[58,66,118,.34],  glow:0},
  {t:385,  c:[255,178,142,.26],glow:.30},
  {t:455,  c:[255,224,176,.10],glow:.20},
  {t:660,  c:[255,250,228,.03],glow:.10},
  {t:790,  c:[255,252,236,.02],glow:.08},
  {t:1010, c:[255,232,182,.08],glow:.16},
  {t:1085, c:[255,180,120,.22],glow:.38},
  {t:1145, c:[246,128,118,.28],glow:.34},
  {t:1235, c:[120,92,150,.36], glow:.14},
  {t:1320, c:[46,52,104,.42],  glow:.04},
  {t:1440, c:[34,46,92,.40],   glow:0},
];
function dayWash(t){
...
function applyDayLight(){
  ...
  ctx.save();
  // colour wash (multiply-ish via plain alpha; kept light for a luminous look)
  ctx.fillStyle=`rgba(${c[0]|0},${c[1]|0},${c[2]|0},${c[3]})`;
  ctx.fillRect(0,0,W,H);
  // warm painterly sun bloom from the upper-left, like late-morning light
  const nt=nightAmt(G.time);
  if(nt<0.9){
    const gx=W*0.30,gy=70;
    const rg=ctx.createRadialGradient(gx,gy,8,gx,gy,520);
    rg.addColorStop(0,`rgba(255,238,196,${0.20+glow*0.5})`);
    rg.addColorStop(.5,`rgba(255,226,170,${0.06+glow*0.2})`);
    rg.addColorStop(1,'rgba(255,226,170,0)');
    ctx.globalCompositeOperation='screen';ctx.fillStyle=rg;ctx.fillRect(0,0,W,H);
    ctx.globalCompositeOperation='source-over';
  }
  // very soft painterly vignette (gentle, never muddy)
  const vg=ctx.createRadialGradient(W/2,H*0.40,H*0.34,W/2,H*0.52,H*0.82);
  vg.addColorStop(0,'rgba(0,0,0,0)');vg.addColorStop(1,`rgba(30,36,58,${0.14+nt*0.22})`);
...
function drawBush(g,x,y,s){
  // soft layered Ghibli shrub: rounded blobs, sunlit rim, little blossoms
  g.save();
  g.fillStyle='rgba(40,70,40,.16)';g.beginPath();g.ellipse(x,y+9*s,13*s,4.5*s,0,0,7);g.fill();
  const blobs=[[-7,3,8],[7,3,8],[0,-5,9.5],[-4,-1,8],[4,-1,8],[0,4,8.5]];
  // shadow base
  g.fillStyle='#3c6038';
  for(const[dx,dy,r] of blobs){g.beginPath();g.arc(x+dx*s,y+(dy+1.5)*s,r*s,0,7);g.fill();}
  // mid green
  g.fillStyle='#5a8a44';
  for(const[dx,dy,r] of blobs){g.beginPath();g.arc(x+dx*s,y+dy*s,r*0.92*s,0,7);g.fill();}
  // sunlit tops (upper-left light)
  g.fillStyle='#7faf56';
  for(const[dx,dy,r] of blobs){g.beginPath();g.arc(x+(dx-2)*s,y+(dy-2.5)*s,r*0.5*s,0,7);g.fill();}
  g.fillStyle='rgba(216,238,170,.7)';
  for(const[dx,dy,r] of [[-4,-6,3],[2,-7,2.6]]){g.beginPath();g.arc(x+dx*s,y+dy*s,r*s,0,7);g.fill();}
  // tangerines / blossoms
  g.fillStyle='#f4a23a';
  for(const[dx,dy] of [[-5,1],[5,-2],[1,5]]){g.beginPath();g.arc(x+dx*s,y+dy*s,1.7*s,0,7);g.fill();}
  g.restore();
}
// a tall fluffy Ghibli tree (single canopy on a slim trunk)
function drawTree(g,x,y,s){
  g.save();
  g.fillStyle='rgba(40,64,38,.20)';g.beginPath();g.ellipse(x+5*s,y+4*s,22*s,7*s,0,0,7);g.fill();
  // trunk
  g.fillStyle='#6e4d2e';g.fillRect(x-3*s,y-22*s,6*s,26*s);
  g.fillStyle='rgba(255,235,190,.25)';g.fillRect(x-3*s,y-22*s,2*s,26*s);
  const blobs=[[-12,-26,12],[12,-26,12],[0,-36,15],[-7,-30,12],[8,-30,12],[0,-22,12]];
  g.fillStyle='#37582f';for(const[dx,dy,r] of blobs){g.beginPath();g.arc(x+dx*s,y+(dy+2)*s,r*s,0,7);g.fill();}
  g.fillStyle='#52803e';for(const[dx,dy,r] of blobs){g.beginPath();g.arc(x+dx*s,y+dy*s,r*0.9*s,0,7);g.fill();}
  g.fillStyle='#79ab53';for(const[dx,dy,r] of blobs){g.beginPath();g.arc(x+(dx-3)*s,y+(dy-3)*s,r*0.46*s,0,7);g.fill();}
  g.fillStyle='rgba(220,242,176,.7)';
  for(const[dx,dy] of [[-8,-40],[2,-42],[-2,-34]]){g.beginPath();g.arc(x+dx*s,y+dy*s,3.4*s,0,7);g.fill();}
  g.restore();
}
...
function buildVillageBG(){
  ...
  // luminous layered grass
  const gg=g.createLinearGradient(0,90,0,SEA_Y);gg.addColorStop(0,'#8cb85e');gg.addColorStop(.55,'#76a64f');gg.addColorStop(1,'#5f9043');
  g.fillStyle=gg;g.fillRect(0,0,W,SEA_Y);
  // broad soft painterly patches of lighter & deeper green
  const rng=mulberry32(20240);
  for(let i=0;i<30;i++){const x=rng()*W,y=96+rng()*(SEA_Y-100),r=40+rng()*70;
    g.fillStyle=`rgba(${150+rng()*40|0},${190+rng()*30|0},${110+rng()*30|0},0.10)`;
    g.beginPath();g.ellipse(x,y,r,r*0.62,rng()*1,0,7);g.fill();}
  for(let i=0;i<14;i++){const x=rng()*W,y=110+rng()*(SEA_Y-120),r=46+rng()*60;
    g.fillStyle='rgba(58,96,52,0.10)';g.beginPath();g.ellipse(x,y,r,r*0.55,rng()*1,0,7);g.fill();}
  // fine sun dapples
  for(let i=0;i<420;i++){const x=rng()*W,y=92+rng()*(SEA_Y-94);
    g.fillStyle=`rgba(${200+rng()*40|0},${224+rng()*20|0},${150+rng()*30|0},${.10+rng()*.16})`;
    g.beginPath();g.ellipse(x,y,1.6+rng()*2.4,1.1+rng()*1.6,0,0,7);g.fill();}
  // soft grass tufts
  g.strokeStyle='rgba(74,108,56,.45)';g.lineWidth=1;
  for(let i=0;i<200;i++){const x=rng()*W,y=100+rng()*(SEA_Y-110);
    g.beginPath();g.moveTo(x,y);g.lineTo(x-1.6,y-5);g.moveTo(x,y);g.lineTo(x+1.6,y-5.5);g.stroke();}
  // drifts of wildflowers (white / coral / gold), Ghibli meadow
  const fcol=['#f6f1e3','#f4a06a','#f3cf52','#e88ca6'];
  for(let i=0;i<150;i++){const x=rng()*W,y=104+rng()*(SEA_Y-116);
    if(y>250&&y<260)continue;
    const col=fcol[(rng()*fcol.length)|0];g.fillStyle=col;
    for(let p=0;p<5;p++){const a=p/5*6.28;g.beginPath();g.arc(x+Math.cos(a)*1.5,y+Math.sin(a)*1.5,1.05,0,7);g.fill();}
    g.fillStyle='#f3cf52';g.beginPath();g.arc(x,y,0.8,0,7);g.fill();}
  // stone-dust paths (soft warm sand)
  const path=(x,y,w,h)=>{
    const pg=g.createLinearGradient(0,y,0,y+h);pg.addColorStop(0,'#ecd6a2');pg.addColorStop(1,'#dcc086');
    g.fillStyle=pg;rr(g,x,y,w,h,Math.min(8,w/2,h/2));g.fill();
    g.fillStyle='rgba(150,116,64,.12)';
    for(let i=0;i<w*h/70;i++){g.beginPath();g.arc(x+rng()*w,y+rng()*h,1+rng()*1.4,0,7);g.fill();}
    g.strokeStyle='rgba(190,160,108,.5)';g.lineWidth=1;rr(g,x+.5,y+.5,w-1,h-1,Math.min(8,w/2,h/2));g.stroke();
  };
...
function drawSea(){
  ...
  sg.addColorStop(0,'#7fd6dc');sg.addColorStop(.45,'#46b2c2');sg.addColorStop(1,'#1f7e98');
...
function drawPerson(x,y,opt){
  ...
  const WHITE='#f6f1e6', WSH='#ddd6c6', BLK='#2b2933', OL='#5b4636';
  ...
  // face
  ctx.fillStyle='#3a2c28';ctx.beginPath();ctx.arc(-2.4,-17.5,1.35,0,7);ctx.arc(3.2,-17.5,1.35,0,7);ctx.fill();
  ctx.fillStyle='rgba(238,128,120,.5)';ctx.beginPath();ctx.arc(-4.6,-15.2,2.1,0,7);ctx.arc(5.2,-15,2.1,0,7);ctx.fill();
  ctx.strokeStyle='rgba(150,90,70,.6)';ctx.lineWidth=1.1;ctx.beginPath();ctx.arc(0.6,-15,2.4,.25,Math.PI-.25);ctx.stroke();
  ctx.restore();
}
...
function drawVillage(){
  ...
  // drifting soft cloud shadows over the meadow
  const tc=performance.now()*0.001;
  ctx.save();ctx.beginPath();ctx.rect(0,90,W,SEA_Y-90);ctx.clip();
  for(let i=0;i<3;i++){
    const span=W+260, cx=((tc*8*(0.5+i*0.25)+i*210)%span)-130;
    const cy=150+i*150;
    ctx.fillStyle='rgba(40,60,46,0.07)';
    for(const[dx,dy,r] of [[-46,0,46],[10,-14,52],[60,4,44],[18,18,40]]){
      ctx.beginPath();ctx.ellipse(cx+dx,cy+dy,r,r*0.5,0,0,7);ctx.fill();}
  }
  ctx.restore();
  drawSea();
  // characters
  ...
  // a couple of little birds gliding across the sky-meadow
  (function(){const tb=performance.now()*0.001;
    ctx.strokeStyle='rgba(60,54,72,.5)';ctx.lineWidth=1.6;ctx.lineCap='round';
    for(let i=0;i<3;i++){
      const span=W+80, bx=((tb*26*(1+i*0.3)+i*180)%span)-40;
      const by=130+i*46+Math.sin(tb*1.2+i)*6;
      const flap=Math.sin(tb*5+i)*3;
      ctx.beginPath();ctx.moveTo(bx-5,by);ctx.quadraticCurveTo(bx-2,by-3-flap,bx,by);
      ctx.quadraticCurveTo(bx+2,by-3-flap,bx+5,by);ctx.stroke();
    }})();
